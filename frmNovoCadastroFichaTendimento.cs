    ...
    public partial class frmNovoCadastroFichaTendimento : Form
    {
        ...
        private void GerarReceitaMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (idExameSelecionado != -1)
            {
                ...
                receita.GerarReceita("RECEITA OPTOMETRICA", idExameSelecionado, true);
            }
            else
            {
        ...
        private void BuscarExamesPorCliente(int idCliente)
        {
            ...
            {
                string query = @"SELECT id_exame, id_cliente, nome_completo, data_do_exame 
                         FROM tb_exames 
                ...
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    ...
                    try
                    {
                        ...
                        dataGridView1.Columns["id_exame"].Visible = true; // Mantenha visível se precisar, ou defina como false para ocultar
                    }
                    catch (Exception ex)
                    {
        ...
        private void dataGridView1_CellClick_1(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                // Exiba o id_exame e a data do exame ou utilize conforme necessário
                MessageBox.Show($"ID do Exame Selecionado: {idExameSelecionado}, Data do Exame: {dataExameSelecionado.ToShortDateString()}");
